...
app.config['MYSQL_PASSWORD'] = 'Anshul1211'
...
def content_based_filtering(product_id):
    ...
    for product_f in cat_product:
        ...
        match_score = 0
        print("ANshul",f_level['product_id'])
        print("Rishu",int(product_id))
        if f_level['product_id'] != int(product_id):
            print("Inside in the loop")
...
def admin_login():
    if request.method == 'POST':
        username = request.form['email']
        password_candidate = request.form['password']
        if username == "joshi.anshul2@gmail.com" and password_candidate == "123" :
            print("Rishu")
            return redirect(url_for('admin'))
        else :
            print("anji")
